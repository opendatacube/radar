...
## dual-pol data (dual pol decomposition).
...
##  > python3.4 dualpol_proc_qsub.py --bbox 130.0 131.0 -21.0 -20.0 --startdate 2018-01-01 --enddate 2018-02-01 --jobs_basename testing
##  > python3.4 dualpol_proc_qsub.py --bbox 130.0 131.0 -21.0 -20.0 --startdate 2018-02-01 --enddate 2018-02-15 --scenes_per_job 3
## 
...
WALLTIME_PER_SCENE = 260    # in [min]; estimate of required walltime to process one scene on 'N_CPUS'
N_CPUS = 8                  # number of cpus to use for each PBS job
MEM_REQ = 48                # in [GB]; MEM (RAM) requirements for PBS job
MEM_JOBFS_REQ = 100         # in [GB]; job's local filesystem MEM requirements (e.g. for DEM file, intermediate data, etc.)
...
def main():
    ...
    parser.add_argument( "--jobs_basename", 
                         help="Base name for submitted PBS jobs. Default is 'dualpol_proc_YYYYMMDD_HHMMSS' (current date and time)." )
    ...
    if cmdargs.jobs_basename is None:
        cmdargs.jobs_basename = 'dualpol_proc_' + str(datetime.now()).split('.')[0].replace('-','').replace(' ','_').replace(':','')
