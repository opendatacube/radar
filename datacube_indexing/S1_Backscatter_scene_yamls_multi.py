...
def prep_dataset(path):
    ...
    for NB in range(int(Num_Bands)):
        ...
        if bandpathnames[NB].endswith('localIncidenceAngle.img'): bands[NB] = 'local_incidence_angle'
    ...
    return {
        ...
        'metadata_information': {'metadata_filename': rel_path_dim, 'incidence_angle_near': ast.literal_eval(IN), 'incidence_angle_far': ast.literal_eval(IF), 'spacing_azimuth': ast.literal_eval(AS), 'spacing_range': ast.literal_eval(RS), 'dem_used': DEM},
...
ARD_output_log_directory = '/g/data/qd04/Cate/yaml_SampleFiles/processing/log_Backscatter/'
# Produce yaml file for each scene in ARD_scene_directory
ARD_scene_directory = '/g/data/qd04/Cate/yaml_SampleFiles/Copernicus_Backscatter/Sentinel-1/C-SAR/GRD/'
