...
## of Sentinel scenes (interferometric coherence). Based on 's1fromNCI.py' from Fang 
## Yuan @ GA on opendatacube/radar GitHub, and based on 'S1_Read_Interferometry_Pairs.py' 
## from Cate Ticehurst @ CSIRO.
##
...
## NCI Testing:
##  > module load python3/3.4.3 python3/3.4.3-matplotlib
##  > python3.4 intcoh_proc_qsub.py --reprocess_existing --express_queue --bbox 146.5 147.5 -38.2 -37.8 --startdate 2018-01-01 --enddate 2018-06-01 --jobs_basename ./log_intcoh_test2/ --base_save_dir /g/data1a/dz56/ga/ga_s1a_c_ard/1-0-0/TESTING/intcoh
##
## VDI Testing:
##  > module load python3/3.4.3 python3/3.4.3-matplotlib
##  > python3.4 intcoh_proc_qsub.py --VDI_jobs --reprocess_existing --bbox 142.3 143.3 -35.7 -34.7 --startdate 2018-06-01 --enddate 2018-07-01 --base_save_dir /g/data/qd04/Eric/tests/test_proc2 --gpt_exec /g/data/qd04/Eric/SNAP/snap/bin/gpt
##  > python3.4 intcoh_proc_qsub.py --VDI_jobs --reprocess_existing --bbox 146.5 147.5 -38.2 -37.8 --startdate 2018-06-01 --enddate 2018-06-31 --jobs_basename ./log_intcoh_test/ --base_save_dir /g/data1a/dz56/ga/ga_s1a_c_ard/1-0-0/TESTING/intcoh --gpt_exec /g/data/qd04/Eric/SNAP/snap/bin/gpt
## 
#####################################################################################
...
XML_GRAPH1 = "intcoh_proc_graph1.xml"           # for consistency checks only -- as defined in JOB_SCRIPT
...
DEF_SAVE_DIR = "/g/data1a/qd04/Copernicus_IntCoherence"     # default base directory of processed scenes outputs
...
WALLTIME_PER_PAIR = 120     # in [min]; estimate of required walltime to process one scene pair on 'N_CPUS'
N_CPUS = 8                  # number of cpus to use for each PBS job
MEM_REQ = 88                # in [GB]; MEM (RAM) requirements for PBS job
MEM_JOBFS_REQ = 120         # in [GB]; job's local filesystem MEM requirements (for DEM file and intermediate data)
...
def main():
    ...
    # input parameters:
    parser = argparse.ArgumentParser(description="Interferometric coherence: create and submit batch jobs on the NCI for processing pairs of Sentinel scenes to ARD data.")
    ...
    parser.add_argument( "--jobs_basename", 
                         help="Base name or dir for submitted PBS jobs. If ends with '/' (i.e. directory), default name will be added to the path. Default name is 'intcoh_proc_YYYYMMDD_HHMMSS' (current date and time)." )
    ...
    if cmdargs.VDI_jobs:    # create shell script for execution on VDI
        ...
        with open(out_fname,'w') as outf:
            outf.write( "#!/bin/bash\n\n" )
            outf.write( "## Batch jobs for INTERFEROMETRIC COHERENCE processing of SAR scenes\n" )
            outf.write( "## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n" )
            outf.write( "##\n")
        ...
        with open(jlist_name,'w') as ln:
            ln.write( "\nBatch jobs for INTERFEROMETRIC COHERENCE processing of SAR scenes\n" )
            ln.write( "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n" )
            ln.write( "Time is: %s \n\n" % str( datetime.now() ) )
        ...
        for job in jobs_arr:    # submit PBS job
            ...
            walltime = WALLTIME_PER_PAIR * len(job)
