...
## Examples:
##  > module load python3/3.4.3 python3/3.4.3-matplotlib
##  > python3.4 dualpol_proc_qsub.py --bbox 130.0 131.0 -21.0 -20.0 --startdate 2018-01-01 --enddate 2018-02-01
##  > python3.4 dualpol_proc_qsub.py --bbox 130.0 131.0 -21.0 -20.0 --startdate 2018-01-01 --enddate 2018-02-01 --jobs_basename /somedir/testing
##  > python3.4 dualpol_proc_qsub.py --bbox 130.0 131.0 -21.0 -20.0 --startdate 2018-01-01 --enddate 2018-02-01 --jobs_basename ./subdir/
##  > python3.4 dualpol_proc_qsub.py --bbox 130.0 131.0 -21.0 -20.0 --startdate 2018-02-01 --enddate 2018-02-15 --scenes_per_job 3   <<=== used for testing! (e.g. LB p. 134)
##  > python3.4 dualpol_proc_qsub.py --bbox 147.0 148.3 -33.8 -33.0 --startdate 2016-08-01 --enddate 2016-12-01 --base_save_dir /g/data/qd04/Copernicus_DualPolDecomp_Lachlan --submit_no_jobs
## 
## Production -- South East Victoria
##  > python3.4 dualpol_proc_qsub.py --bbox 146.5 147.5 -38.2 -37.8 --startdate 2018-01-01 --enddate 2018-07-01 --jobs_basename ./log/ --base_save_dir /g/data1a/dz56/ga/ga_s1a_c_ard/1-0-0/Copernicus_DualPolDecomp_SEVic
## Production -- North West Victoria
##  > python3.4 dualpol_proc_qsub.py --bbox 142.3 144.0 -35.75 -34.5 --startdate 2018-01-01 --enddate 2018-07-01 --jobs_basename ./log/ --base_save_dir /g/data1a/dz56/ga/ga_s1a_c_ard/1-0-0/Copernicus_DualPolDecomp_NWVic
## 
## NCI Testing:
##  > module load python3/3.4.3 python3/3.4.3-matplotlib
##  > python3.4 dualpol_proc_qsub.py --reprocess_existing --express_queue --bbox 146.5 147.5 -38.2 -37.8 --startdate 2018-06-01 --enddate 2018-07-01 --jobs_basename ./log_dualpol_test/ --base_save_dir /g/data1a/dz56/ga/ga_s1a_c_ard/1-0-0/Copernicus_dualpol_TESTING
##  > python3.4 dualpol_proc_qsub.py --reprocess_existing --express_queue --bbox 146.5 147.5 -38.2 -37.8 --startdate 2018-01-01 --enddate 2018-06-01 --jobs_basename ./log_dualpol_test2/ --base_save_dir /g/data1a/dz56/ga/ga_s1a_c_ard/1-0-0/TESTING/dualpol
## 
## VDI Testing:
##  > module load python3/3.4.3 python3/3.4.3-matplotlib
##  > python3.4 dualpol_proc_qsub.py --reprocess_existing --bbox 142.3 143.3 -35.7 -34.7 --startdate 2018-06-01 --enddate 2018-07-01 --base_save_dir /g/data/qd04/Eric/tests/test_proc2 --VDI_jobs --gpt_exec /g/data/qd04/Eric/SNAP/snap/bin/gpt
##  > python3.4 dualpol_proc_qsub.py --VDI_jobs --reprocess_existing --bbox 146.5 147.5 -38.2 -37.8 --startdate 2018-06-01 --enddate 2018-06-10 --jobs_basename ./log_dualpol_test/ --base_save_dir /g/data1a/dz56/ga/ga_s1a_c_ard/1-0-0/Copernicus_dualpol_TESTING3 --gpt_exec /g/data/qd04/Eric/SNAP/snap/bin/gpt
## 
#####################################################################################
...
JOB_SCRIPT = "dualpol_proc.sh"  # name of PBS shell script to carry out the dual-pol processing
DEM_MAKE_FILE = "make_DEM.py"                   # for consistency checks only -- as used in JOB_SCRIPT
XML_GRAPH1 = "dualpol_proc_graph1.xml"          # for consistency checks only -- as defined in JOB_SCRIPT
...
DEF_SAVE_DIR = "/g/data1a/qd04/Copernicus_DualPolDecomp"     # default base directory of processed scenes outputs
...
DEF_SCENES_PER_JOB = 5      # default nr of scenes per job submitted to PBS
WALLTIME_PER_SCENE = 120    # in [min]; estimate of required walltime to process one scene on 'N_CPUS'
N_CPUS = 8                  # number of cpus to use for each PBS job
MEM_REQ = 88                # in [GB]; MEM (RAM) requirements for PBS job
MEM_JOBFS_REQ = 64          # in [GB]; job's local filesystem MEM requirements (for DEM file and intermediate data)
...
def main():
    ...
    # input parameters:
    parser = argparse.ArgumentParser(description="Dual-pol decomposition: create and submit batch jobs on the NCI for processing Sentinel scenes to ARD data.")
    ...
    parser.add_argument( "--jobs_basename", 
                         help="Base name or dir for submitted PBS jobs. If ends with '/' (i.e. directory), default name will be added to the path. Default name is 'dualpol_proc_YYYYMMDD_HHMMSS' (current date and time)." )
    ...
    if cmdargs.VDI_jobs:    # create shell script for execution on VDI
        ...
        with open(out_fname,'w') as outf:
            outf.write( "#!/bin/bash\n\n" )
            outf.write( "## Batch jobs for DUAL POL. DECOMPOSITION processing of SAR scenes\n" )
            outf.write( "## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n" )
            outf.write( "##\n")
